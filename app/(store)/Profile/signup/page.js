...
export default function SignupPage() {
  ...
  const handleSignup = (e) => {
    ...
    if (!mobile || !email || !password) {
      alert('Please fill in registration credentials');
      return;
    }
    ...
    <div style={{ padding: '30px', maxWidth: '400px', margin: '60px auto', borderRadius: 'var(--borderRadius)', background: 'var(--cardBg)', boxShadow: 'var(--shadowLight)' }}>
      ...
      <form onSubmit={handleSignup}>
        <div style={{ marginBottom: '15px' }}>
          <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: 'var(--textMuted)', marginBottom: '6px', textTransform: 'uppercase' }}>Mobile Number</label>
          <input 
            type="text" 
      ...
      <p style={{ marginTop: '20px', textAlign: 'center', fontSize: '12px', color: 'var(--textMuted)' }}>
        Already have an account? <a href="/Profile/profile" style={{ color: 'var(--accentColor)', textDecoration: 'none', fontWeight: '700' }}>Log In</a>
      </p>
    </div>
  );
}
